...
def train_and_evaluate_ensemble(X, y, use_pca_features=False, model_type='random_forest'):
    ...
    print(f"\n=== Обучение и тестирование АНСАМБЛЯ Random Forest (Leave-One-Out) ===")
...
def calculate_metrics(y_true, y_pred, y_proba):
    """
    Вычисление всех метрик качества: Accuracy, FAR, FRR, EER.
...
def print_results(results):
    ...
    print(f"Истинно 0 (чужие)    {results['tn']}              {results['fp']}")
    print(f"Истинно 1 (свои)     {results['fn']}              {results['tp']}")
    print(f"\nFAR (False Acceptance Rate): {results['far']:.4f} ({results['far']*100:.2f}%)")
...
def compare_models(results_list):
    ...
    print("="*80)
    print(f"{'Модель':<40} {'Accuracy':<12} {'FAR':<12} {'FRR':<12} {'EER':<12}")
    print("-"*80)
    ...
    print(f"\n🏆 Лучшая модель по EER: {best_model['model_name']}")
    print(f"   EER = {best_model['eer']*100:.2f}%, Accuracy = {best_model['accuracy']*100:.2f}%")
