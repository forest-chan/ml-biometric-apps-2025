...
if __name__ == "__main__":
    ...
    # Итоговые выводы для анализа
    print("\n" + "="*70)
    print("АНАЛИЗ РЕЗУЛЬТАТОВ")
    print("="*70)
    print(f"\n1. КАЧЕСТВО МОДЕЛИ:")
    print(f"   - Точность (Accuracy): {results['accuracy']*100:.2f}%")
    print(f"   - FAR (False Acceptance Rate): {results['far']*100:.2f}%")
    print(f"   - FRR (False Rejection Rate): {results['frr']*100:.2f}%")
    print(f"\n   Почему эти цифры НЕ репрезентативны для реального мира:")
    print(f"   • Крайне малый размер выборки ({len(y)} образцов)")
    print(f"   • Нет учёта разнообразия условий (освещение, углы, качество)")
    print(f"   • Отсутствует разнообразие демографических групп")
    print(f"   • Leave-One-Out на 10 образцах даёт высокую дисперсию оценки")
    print(f"\n2. РАЗМЕРНОСТЬ ДАННЫХ:")
    print(f"   - Исходная: {X.shape[1]} признаков (пикселей)")
    print(f"   - После PCA: {X_pca.shape[1]} признаков (главных компонент)")
    print(f"   - Сжатие: {(1 - X_pca.shape[1]/X.shape[1])*100:.2f}% сокращение размерности")
    print("\n" + "="*70)
